...
// ══════════════════════════════════════════
//  BG MUSIC — RPG Battle Theme
// ══════════════════════════════════════════

// Lead melody: heroic minor-key RPG feel
const MELODY = [
  [440,0.00],[494,0.18],[523,0.36],[587,0.54],
  [659,0.72],[587,0.90],[523,1.08],[494,1.26],
  [440,1.44],[392,1.62],[440,1.80],[494,2.07],
  [523,2.34],[587,2.52],[659,2.79],[784,3.06],
  [740,3.33],[698,3.51],[659,3.69],[622,3.87],
  [587,4.05],[554,4.23],[523,4.41],[494,4.59],
  [440,4.77],[392,4.95],[349,5.13],[392,5.31],
  [440,5.58],[523,5.85],[440,6.12],[392,6.39],
];

// Harmony layer — a third above for RPG thickness
const HARMONY = [
  [523,0.00],[587,0.18],[659,0.36],[698,0.54],
  [784,0.72],[698,0.90],[659,1.08],[587,1.26],
  [523,1.44],[494,1.62],[523,1.80],[587,2.07],
  [659,2.34],[698,2.52],[784,2.79],[932,3.06],
  [880,3.33],[831,3.51],[784,3.69],[740,3.87],
  [698,4.05],[659,4.23],[622,4.41],[587,4.59],
  [523,4.77],[494,4.95],[440,5.13],[494,5.31],
  [523,5.58],[659,5.85],[523,6.12],[494,6.39],
];

// Bass — slow, weighty RPG root pattern
const BASS = [
  [110,0.00],[110,0.36],[131,0.72],[110,1.08],
  [110,1.44],[98, 1.80],[110,2.16],[131,2.52],
  [147,2.88],[131,3.24],[123,3.60],[117,3.96],
  [110,4.32],[98, 4.68],[110,5.04],[131,5.40],
  [110,5.76],[131,6.12],
];

// Drums
const KICK_TIMES  = [0.00,0.72,1.44,2.16,2.88,3.60,4.32,5.04,5.76,6.48];
const SNARE_TIMES = [0.36,1.08,1.80,2.52,3.24,3.96,4.68,5.40,6.12];

const BEAT_DUR = 6.75;

function rpgKick(startSec) {
  try {
    const c = getAC(), t = c.currentTime + startSec;
...
function startBG() {
  ...
  function scheduleBeat() {
    ...
    MELODY.forEach(([f, s])  => note(f, s, 0.17, 0.10, 'square',   0.01, 0.06));
    HARMONY.forEach(([f, s]) => note(f, s, 0.16, 0.055,'triangle', 0.02, 0.08));
    BASS.forEach(([f, s])    => note(f, s, 0.34, 0.09, 'sawtooth', 0.01, 0.15));
    KICK_TIMES.forEach(s  => rpgKick(s));
    SNARE_TIMES.forEach(s => rpgSnare(s));
    bgLoop = setTimeout(scheduleBeat, (BEAT_DUR - 0.05) * 1000);
  }
  scheduleBeat();
}
